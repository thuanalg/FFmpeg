...
if(!pdata) break; spllog(1, "f(w,h,fmt,sample_rate, nb_samples): (%d, %d, %d, %d, %d), linesize[0-3]: (%d, %d, %d, %d), [%x, %x, %x, %x]", \
            (__fr__)->width  , \
            (__fr__)->height , \
            (__fr__)->format , \
            (__fr__)->sample_rate , (__fr__)->nb_samples,\
            (__fr__)->linesize[0], (__fr__)->linesize[1],\
            (__fr__)->linesize[2], (__fr__)->linesize[3], \
...
#ifndef spl_d4slice
#define spl_d4slice(__name__, __x__, __y__, __bytes__) do{ \
    unsigned int *pdata = (unsigned int*)(__bytes__); if(!pdata) break;\
    spllog(1, "(name, x, y)=(%s, %d, %d) bytes(0,1,2,3): (%x, %x, %x, %x)", \
            (__name__) , \
            (__x__) , (__y__) , \
            pdata[0] , pdata[1] , \
            pdata[2] , pdata[3] );  \
